//gcc -Wall queue_func6.c -o q6 -lpaho-mqtt3c -ljson-c -lmysqlclient -lpthread `pkg-config --cflags gtk+-3.0` `pkg-config --libs gtk+-3.0` 
...
GtkWidget *Entryq, *Entryday, *Entrymonth, *Entryyear, *Entryname, *Entrynumpeople ,*Entrytel, *Entrystatus;
...
static pthread_mutex_t lock_res = PTHREAD_MUTEX_INITIALIZER;
...
void *Thread_job()
{
    ...
    while(1)
    {
        ...
        int* remain_recorder = calloc(size,sizeof(int));
        int* queue_recorder = calloc(size,sizeof(int));
        int i =0;
        //printf("size:%d\n",size);
        while (i<size)
        {
        ...
        //printf("%d\n",queue_recorder[size-1]);
        send_showR(queue_recorder[size-1]);          
...
int msgarrvd(void *context, char *topicName, int topicLen, MQTTClient_message *message)
{
  ...
  //INSERT DATA
    ...
    respond.sent = 1;
    //send_res(obj[3],recent_queue, remain);
    memset(query,0,strlen(query));
...
respond.sent =0;
  ...
  exit(1);
   ...
   if(pthread_mutex_init(&lock_res,NULL))
  {        
  printf("Lock init Failed!!\n");
  exit(1);
  }
 ...
 int retval = pthread_create(&chk_almost, NULL,Thread_job,NULL );
  ...
  int r = system("x-terminal-emulator -e \"/home/hill/Desktop/Queuing/open_broker.sh\"");
  ...
  r = system("/home/hill/Desktop/Queuing/kill_broker.sh");
	return 0;
}
static gboolean btn_clicked(GtkWidget *widget, gpointer parent){
  GtkWidget *dialog;
...
void confirmbutton_callback(GtkWidget *b, gpointer data){
  ...
  sprintf(query,"SELECT c_id FROM queue WHERE id = %d",sel);
  if (mysql_query(con, query))
  {
        finish_with_error(con);
  }
...
void activate( GtkApplication *app, gpointer user_data ){
  ...
  //tab2
    ...
    GtkWidget *scrwin2, *frame2, *vbox2, *hboxlabel, *hboxentry, *eventbox2, *listbox2, *hboxdate,
    ...
    //gtk_entry_set_placeholder_text(GTK_ENTRY(Entrytel),"Tel. number");
    ...
    //gtk_entry_set_placeholder_text(GTK_ENTRY(Entrynumpeople),"Number of People");
    ...
    //gtk_entry_set_placeholder_text(GTK_ENTRY(Entrystatus),"Status");
