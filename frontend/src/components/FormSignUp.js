...
var client = mqtt.connect("ws://25.52.150.197:9001",conoption);
...
function ValidateInfo(values){
        ...
        if (!values.Name.trim()) {
            ...
            //credit: https://www.codegrepper.com/code-examples/javascript/how+to+special+characters+not+allowed+in+javascript+validation
            errors.Name = 'Please enter a valid name that not contain any special characters or numbers.';
        }
    
        if(!values.tel.trim()){
            errors.tel = 'Telephone numbers requied.';
        }
...
const FormSignUp = () => {
    ...
    //const {handleCancel, handleChange, handleSubmit, values, state, errors} = UseForm(ValidateInfo, client,client_id,is_connected,pub_topic);
        ...
        const handleSubmit = (e) => {
            ...
            {
                Swal.fire({
                    title: 'Sorry, please come back later.',
                    text: "opening hour is 10.00 - 22.00.",
                    icon: 'warning',
                    confirmButtonColor: '#3085d6',
            ...
            Swal.fire({
                ...
                if (result.isConfirmed && !errors.Name && !errors.num_people && !errors.tel) {
                    ...
                    Swal.fire({
                        title: 'Oops...',
                        text: "Please check your form for errors and submit again",
                        icon: 'question',
                        showCloseButton: true,
                    })
                    setErrors(errors);
